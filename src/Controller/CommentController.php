...
class CommentController extends AbstractController
{
    ...
    #[Route('/comment/ajax-create', name: 'ajax_create_comment', methods: ['POST'])]
        ...
        if ($request->isXmlHttpRequest()) {
            ...
            switch ($commentedEntityType) {
                ...
                case 'projectPresentation':
                    $presentation = $ppRepo->findOneBy(['id' => $commentedEntityId]);
            ...
            $newComment
                ...
                ->setStatus(\App\Enum\CommentStatus::Approved)
                ->setContent($commentContent);

            $validation = $commentService->validateComment($newComment, $formTimeLoaded, $honeyPot);
            ...
            if (!is_null($repliedSiblingCommentId)) {
                ...
                $parentComment = $manager->getRepository(Comment::class)->findOneBy(['id' => $parentCommentId]);

                $newComment->setParent($parentComment);
                
                $repliedSiblingComment = $manager->getRepository(Comment::class)->findOneBy(['id' => $repliedSiblingCommentId]);
                ...
                if ($repliedSiblingCommentId !== $parentCommentId) {
                    ...
                    $newComment->setRepliedUser($repliedSiblingComment?->getCreator());
   ...
   #[Route('/comments/remove/', name: 'ajax_remove_comment', methods: ['POST'])]
